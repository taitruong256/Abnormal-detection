...
if __name__ == "__main__":
    df, train_loader, train_open_loader, train_close_loader, val_loader, val_open_loader, val_close_loader = load_data(BATCH_SIZE)
    ...
    best_f1_open = 0.0
    best_model_path = r'E:\data\best_model.pth'
    ...
    if IS_TRAIN:
        for epoch in range(NUM_EPOCHS):
            ...
            all_latent_vectors, mean_vector = train_vae(vae, device, train_close_loader, train_open_loader, optimizer, BETA, LAMBDA_, LATENT_DIM)
            
            val_losses = evaluate_vae(vae, device, val_close_loader, val_open_loader, LATENT_DIM, BETA, LAMBDA_)
    ...
    evaluate_and_plot_samples(df, vae, mean_vector, weibull_model, NUM_CLASSES, OMEGA_T, device, output_dir)
